...
function render(input_str, output_name, obj) {
...
const configTemplate = readFileSync(path.resolve(templatesPath, 'chain.toml.template'));
const dockerComposeTemplate = readFileSync(path.resolve(templatesPath, 'docker-compose.yml.template'))
const genesisTemplate = readFileSync(path.resolve(templatesPath, 'genesis.toml.template'))
